...
On the request branch, commit a request document at `docs/inbox/{YYYYMMDD_HHMM}_{requester}_request.md`, then push the branch to the target repo's own origin.
...
- When the abstraction is finalized, commit a response document on the same branch at `docs/outbox/{YYYYMMDD_HHMM}_{requester}_response.md`.
...
- Read the response document in `docs/outbox/`.
...
`docs/inbox/` and `docs/outbox/` are append-only. Once merged to main, a request/response pair is never edited — a later change is a new request/response pair that references the earlier one, so the history stays a true audit trail.
